...
async function init() {
  ...
  app.post('/', async (req, res) => {
    ...
    const positionResult = await pool.query(positionNameInsertQuery, [positionName]);
    ...
    const prefixResult = await pool.query(prefixNameInsertQuery, [prefixName]);
    ...
    const personalResult = await pool.query(personalInfoInsertQuery, [prefixId, fullName, nickName, formattedBirthday, positionId, approve]);
    ...
    await pool.query(fileInfoInsertQuery, [personalId, selectedFile]);
